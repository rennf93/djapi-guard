...
class ErrorResponseFactory:
    ...
    def create_error_response(
        ...
        Returns:
            HttpResponse object with custom message and security headers
        """
        custom_message = self.context.config.custom_error_responses.get(
            status_code, default_message
        )
        response = HttpResponse(custom_message, status=status_code)
    ...
    def create_https_redirect(self, request: HttpRequest) -> HttpResponse:
        """
    ...
    def apply_security_headers(
        self, response: HttpResponse, request_path: str | None = None
    ) -> HttpResponse:
        """
        Apply security headers to response.

        Args:
            response: HttpResponse object to add headers to
    ...
    def apply_cors_headers(self, response: HttpResponse, origin: str) -> HttpResponse:
        """
        Apply CORS headers to response.

        Args:
            response: HttpResponse object to add CORS headers to
            origin: Origin header value from request

        Returns:
            HttpResponse with CORS headers added
        """
        headers_config = self.context.config.security_headers
        if headers_config and headers_config.get("enabled", True):
    ...
    def apply_modifier(self, response: HttpResponse) -> HttpResponse:
        """
    ...
    def process_response(
        ...
        process_behavioral_rules: Callable[
            [HttpRequest, HttpResponse, str, RouteConfig], None
        ]
        | None = None,
    ) -> HttpResponse:
        """
