...
class SecurityCheck(ABC):
    ...
    def __init__(self, middleware: "DjangoAPIGuard") -> None:
        self.middleware = middleware
        if middleware.config is None:
            raise RuntimeError("DjangoAPIGuard must be initialized with config")
        self.config: SecurityConfig = middleware.config
        if middleware.logger is None:
            raise RuntimeError("DjangoAPIGuard must be initialized with logger")
    ...
    def check(self, request: HttpRequest) -> HttpResponse | None:
        ...
        Returns:
            HttpResponse if the check fails and request should be blocked.
    ...
    def create_error_response(
        ...
        Returns:
            HttpResponse object with appropriate status and headers.
        """
