...
def setup_custom_logging(
    ...
    """
    Setup custom logging for Django API Guard.
    ...
    """
    logger = logging.getLogger("djangoapi_guard")
...
def extract_client_ip(
    ...
    1. If no trusted proxies are defined, the connecting IP
       (request.META['REMOTE_ADDR']) is always used
    ...
    """
    connecting_ip: str = str(request.META.get("REMOTE_ADDR", ""))
...
def _extract_request_context(request: HttpRequest) -> dict[str, Any]:
    client_ip: str = str(request.META.get("REMOTE_ADDR", "") or "unknown")
...
def _extract_ip_from_request(request: str | HttpRequest) -> str:
...
def check_ip_country(
    ...
    Args:
        request (str | HttpRequest):
            The Django request object or IP string.
        config (SecurityConfig):
            The security configuration object.
        geo_ip_handler (GeoIPHandler):
...
def _fallback_pattern_check(value: str) -> tuple[bool, str]:
    for pattern, _ctx in sus_patterns_handler.get_all_compiled_patterns():
        try:
            ...
            logging.debug("Fallback pattern check failed", exc_info=True)
            continue
    return False, ""
...
def _check_query_params(
    request: HttpRequest, client_ip: str, correlation_id: str
) -> tuple[bool, str]:
...
def _check_headers_for_threats(
    ...
    for meta_key, value in request.META.items():
        ...
        if header_name not in _EXCLUDED_HEADERS:
            detected, trigger = _check_request_component(
                str(value),
            ...
            if detected:
                return True, f"Header '{header_name}': {trigger}"
    return False, ""
...
def detect_penetration_attempt(request: HttpRequest) -> tuple[bool, str]:
    ...
    Args:
        request (HttpRequest):
            The Django request object to analyze.
