...
def reset_ipinfo_singleton() -> Generator[None, None, None]:
    original_instance = IPInfoManager._instance
    IPInfoManager._instance = None
    yield
    IPInfoManager._instance = original_instance
...
def test_security_config_none_whitelist() -> None:
    ...
    config = SecurityConfig(enable_redis=False, enable_agent=False, whitelist=None)
...
def test_none_cloud_providers() -> None:
    ...
    config = SecurityConfig(
        enable_redis=False, enable_agent=False, block_cloud_providers=None
    )
...
def test_missing_ipinfo_token() -> None:
    ...
    with pytest.raises(ValueError):
        SecurityConfig(enable_redis=False, enable_agent=False, blocked_countries=["US"])

    with pytest.raises(ValueError):
        SecurityConfig(
            enable_redis=False, enable_agent=False, whitelist_countries=["US"]
        )
...
def test_geo_ip_handler_validation() -> None:
    ...
    config = SecurityConfig(
        enable_redis=False, enable_agent=False, geo_ip_handler=ipinfo
    )
    assert config.geo_ip_handler == ipinfo

    valid_instance = ValidGeoIPHandler()
    config = SecurityConfig(
        enable_redis=False, enable_agent=False, geo_ip_handler=valid_instance
    )
    assert config.geo_ip_handler == valid_instance

    config = SecurityConfig(enable_redis=False, enable_agent=False, geo_ip_handler=None)
    ...
    with pytest.raises(ValueError):
        SecurityConfig(
            enable_redis=False, enable_agent=False, geo_ip_handler=invalid_handler
        )
...
def test_validate_trusted_proxies() -> None:
    ...
    with pytest.raises(ValueError, match="Invalid proxy IP or CIDR range"):
        SecurityConfig(
            enable_redis=False, enable_agent=False, trusted_proxies=["invalid-ip"]
        )

    config = SecurityConfig(enable_redis=False, enable_agent=False, trusted_proxies=[])
...
def test_validate_proxy_depth() -> None:
    ...
    config = SecurityConfig(
        enable_redis=False, enable_agent=False, trusted_proxy_depth=2
    )
    assert config.trusted_proxy_depth == 2

    with pytest.raises(ValueError, match="trusted_proxy_depth must be at least 1"):
        SecurityConfig(enable_redis=False, enable_agent=False, trusted_proxy_depth=0)
...
def test_validate_agent_config_dynamic_rules_without_agent() -> None:
    ...
    with pytest.raises(ValueError, match="enable_agent must be True"):
        SecurityConfig(
            enable_redis=False, enable_dynamic_rules=True, enable_agent=False
        )
...
def test_to_agent_config_no_api_key() -> None:
    ...
    config = SecurityConfig(enable_redis=False, enable_agent=False, agent_api_key=None)
    assert config.to_agent_config() is None
