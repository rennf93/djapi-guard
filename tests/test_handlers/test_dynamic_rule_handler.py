"""Comprehensive tests for djangoapi_guard/handlers/dynamic_rule_handler.py."""
...
from djangoapi_guard.models import DynamicRules, SecurityConfig
...
def cleanup_singleton() -> Generator[None, None, None]:
    ...
    yield
    instance: DynamicRuleManager | None = getattr(DynamicRuleManager, "_instance", None)
    ...
    def test_loop_handles_exception(
        ...
        with patch.object(mgr, "update_rules", side_effect=RuntimeError("boom")):
            ...
            with caplog.at_level(
                logging.ERROR, logger="djangoapi_guard.handlers.dynamic_rule"
            ):
                t = threading.Thread(target=mgr._rule_update_loop, daemon=True)
                t.start()
                time.sleep(0.3)
...
class TestUpdateRules:
    ...
    def test_handles_exception_in_update(
        ...
        with caplog.at_level(
            logging.ERROR,
            logger="djangoapi_guard.handlers.dynamic_rule",
        ):
            manager.update_rules()
...
class TestSendRuleReceivedEvent:
    ...
    def test_handles_import_error(
        ...
        manager.agent_handler = mock_agent_handler
        mock_agent_handler.send_event.side_effect = Exception("Agent error")
        with caplog.at_level(
            logging.ERROR, logger="djangoapi_guard.handlers.dynamic_rule"
        ):
            manager._send_rule_received_event(sample_rules)
