...
def test_cors_headers_with_security_headers(reset_headers_manager: None) -> None:
    ...
    try:
        ...
        request = factory.get("/test", HTTP_ORIGIN="https://example.com")
        response = middleware(request)

        assert response.status_code == 200
        assert response["Access-Control-Allow-Origin"] == "https://example.com"
        assert "GET, POST" in response["Access-Control-Allow-Methods"]
...
def test_cors_preflight_options_request(reset_headers_manager: None) -> None:
    ...
    try:
        middleware = DjangoAPIGuard(dummy_view)
        factory = RequestFactory()
        request = factory.options("/test", HTTP_ORIGIN="https://example.com")
        response = middleware(request)
