...
class TestReferrerCheckPassiveModeUnit:
    ...
    def test_handle_missing_referrer_passive_mode_unit(self) -> None:
        ...
        with patch(
            "djangoapi_guard.core.checks.implementations.referrer.log_activity",
...
class TestAuthenticationCheckPassiveModeUnit:
    ...
    def test_authentication_check_passive_mode_unit(self) -> None:
        ...
        with patch(
            "djangoapi_guard.core.checks.implementations.authentication.log_activity",
            return_value=MagicMock(),
        ):
...
class TestEmergencyModeCheckPassiveModeUnit:
    ...
    def test_emergency_mode_check_no_client_ip_extracts_unit(self) -> None:
        ...
        request = Mock()
        request._guard_client_ip = None

        with patch(
            "djangoapi_guard.core.checks.implementations.emergency_mode.extract_client_ip",
            return_value="8.8.8.8",
        ):
            with patch(
                "djangoapi_guard.core.checks.implementations.emergency_mode.log_activity",
                return_value=MagicMock(),
            ):
                result = check.check(request)
    ...
    def test_emergency_mode_check_passive_mode_unit(self) -> None:
        ...
        request = Mock()
        request._guard_client_ip = "8.8.8.8"
...
class TestRequestSizeContentCheckPassiveModeUnit:
    ...
    def test_check_request_size_limit_passive_mode_unit(self) -> None:
        ...
        with patch(
            "djangoapi_guard.core.checks.implementations.request_size_content.log_activity",
            return_value=MagicMock(),
        ):
...
class TestRequiredHeadersCheckPassiveModeUnit:
    ...
    def test_required_headers_check_passive_mode_unit(self) -> None:
        ...
        with patch(
            "djangoapi_guard.core.checks.implementations.required_headers.log_activity",
            return_value=MagicMock(),
        ):
            result = check.check(request)
            assert result is None
...
class TestUserAgentRouteSpecificBlock:
    ...
    def test_route_specific_blocked_user_agent(self) -> None:
        ...
        from djangoapi_guard.core.checks.implementations.user_agent import (
            UserAgentCheck,
        )
        ...
        request = Mock()
        request.META = {"HTTP_USER_AGENT": "evilbot"}
        request._guard_is_whitelisted = False
        request._guard_route_config = route_config

        with patch(
            "djangoapi_guard.core.checks.implementations.user_agent.log_activity",
        ):
            with patch(
                "djangoapi_guard.core.checks.implementations.user_agent.check_user_agent_allowed",
                return_value=False,
            ):
                result = check.check(request)
                assert result is not None
