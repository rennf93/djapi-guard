...
class TestIpSecurityEdgeCases:
    ...
    def test_check_banned_ip_passive_mode(
        ...
        with patch(
            "djangoapi_guard.core.checks.implementations.ip_security.ip_ban_manager"
        ) as mock_ban_mgr:
            mock_ban_mgr.is_ip_banned = MagicMock(return_value=True)

            with patch(
                "djangoapi_guard.core.checks.implementations.ip_security.log_activity"
            ) as mock_log:
    ...
    def test_check_route_ip_restrictions_passive_mode(
        ...
        with patch(
            "djangoapi_guard.core.checks.implementations.ip_security.check_route_ip_access"
        ) as mock_check:
    ...
    def test_check_global_ip_restrictions_passive_mode(
        ...
        with patch(
            "djangoapi_guard.core.checks.implementations.ip_security.is_ip_allowed"
        ) as mock_allowed:
