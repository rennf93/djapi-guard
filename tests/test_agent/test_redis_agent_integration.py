...
class TestRedisManagerAgentIntegration:
    ...
    def test_send_redis_event_exception_handling(
        ...
        caplog.set_level(logging.ERROR, logger="djangoapi_guard.handlers.redis")
    ...
    def test_get_connection_closed_error_with_agent(self) -> None:
        ...
        assert "Redis connection closed" in str(exc_info.value)
...
def patch_security_event() -> Any:
    with patch(
        "djangoapi_guard.handlers.redis_handler.SecurityEvent", create=True
    ) as mock_event:
