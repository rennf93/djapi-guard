...
class TestSusPatternsManagerAgentIntegration:
    ...
    def test_send_pattern_event_exception_handling(
        ...
        caplog.set_level(logging.ERROR, logger="djangoapi_guard.handlers.suspatterns")
...
def cleanup_suspatterns_singleton() -> Generator[None, None, None]:
    ...
    current_instance: SusPatternsManager | None = getattr(
        SusPatternsManager, "_instance", None
    )
    if current_instance is not None:
        current_instance.custom_patterns.clear()
        current_instance.compiled_custom_patterns.clear()

        original_len = len(SusPatternsManager.patterns)
        while len(current_instance.patterns) > original_len:
            current_instance.patterns.pop()  # pragma: no cover
            current_instance.compiled_patterns.pop()  # pragma: no cover
