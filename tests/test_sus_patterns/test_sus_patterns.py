"""
Comprehensive tests for the SusPatternsManager handler.
"""
...
from djangoapi_guard.handlers.suspatterns_handler import (
    SusPatternsManager,
...
def test_get_compiled_patterns_separation() -> None:
    ...
    default_matched = any(p.search(test_default_string) for p, _ in default_compiled)
    assert default_matched

    test_custom_string = "custom_test_pattern_456"
    custom_matched = any(p.search(test_custom_string) for p, _ in custom_compiled)
