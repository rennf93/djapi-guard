...
from djangoapi_guard.detection_engine.monitor import (
    PatternStats,
    PerformanceMetric,
    PerformanceMonitor,
)
...
def test_statistical_anomaly_single_measurement_after_len_check() -> None:
    """Test _detect_statistical_anomaly:
    len(recent_times) <= 1 after list()."""
    monitor = PerformanceMonitor(anomaly_threshold=2.0)

    pattern = "single_measurement_pattern"
    stats = PatternStats(pattern=pattern)

    # Create a mock deque-like object that reports len >= 10
    # but iterates to only 1 element, to cover _detect_statistical_anomaly
    mock_times = MagicMock()
    mock_times.__len__ = MagicMock(return_value=10)
    mock_times.__iter__ = MagicMock(return_value=iter([0.01]))
    stats.recent_times = mock_times
    monitor.pattern_stats[pattern] = stats
